    ...
    <div className="bg-slate-50 dark:bg-slate-900 min-h-full pb-24 animate-in fade-in duration-300 transition-colors duration-300">
      ...
      <header className="flex justify-between items-center p-4 bg-white dark:bg-slate-950 sticky top-0 z-10 border-b border-slate-100 dark:border-slate-800/80 shadow-sm transition-colors duration-300">
        ...
        <div className="flex flex-col items-center">
          ...
          <h1 className="text-[13px] font-black text-slate-900 dark:text-white uppercase tracking-wider">Padma Bridge Infographic</h1>
        </div>
      ...
      {/* Outer padding 24px as requested */}
      <div className="p-6 space-y-7">
        
        {/* Title Block with Exact Requested Typography Sizes */}
        <div className="text-center space-y-1.5">
          <h1 className="text-[32px] font-black text-slate-900 dark:text-white uppercase leading-[1.15] tracking-tight">
            PADMA BRIDGE
          </h1>
          <p className="text-[17px] font-medium text-slate-500 dark:text-slate-400 leading-tight">
        ...
        {/* TAB 1: KEY FACTS */}
        {activeTab === 'Facts' && (
          <div className="space-y-7 animate-in slide-in-from-bottom-2 duration-300">
            
            {/* Section heading: Key Facts (Size: 20 px) */}
            <div className="space-y-3">
              <h2 className="text-[20px] font-bold text-slate-900 dark:text-white uppercase tracking-tight flex items-center gap-2">
                <span>📊</span> KEY FACTS
              </h2>
              <p className="text-[15px] font-normal text-slate-600 dark:text-slate-300 leading-relaxed">
            ...
            <div className="grid grid-cols-2 gap-4">
              {FACTS.map((fact, index) => (
                <div 
                  key={index} 
                  className="bg-white dark:bg-slate-950 p-4 rounded-[12px] border border-slate-100 dark:border-slate-800/80 shadow-sm space-y-2 transition duration-300 hover:border-blue-200 dark:hover:border-blue-900/50"
                >
                  <div className="flex justify-between items-center">
                    <span className="text-lg">{fact.icon}</span>
                    <span className="text-[10px] font-black text-blue-500 bg-blue-50 dark:bg-blue-950/40 px-2 py-0.5 rounded-full uppercase">GK Stat</span>
                  </div>
                  <div>
                    {/* Fact Number: 24 px */}
                    <p className="text-[24px] font-bold text-blue-600 dark:text-blue-400 leading-none">
                      {fact.number}
                    </p>
                    {/* Fact Label: 14 px */}
                    <p className="text-[14px] font-medium text-slate-500 dark:text-slate-400 mt-1 leading-snug">
                      {fact.label}
                    </p>
                  </div>
                </div>
            ...
            <div className="bg-white dark:bg-slate-950 p-5 rounded-[12px] border border-slate-100 dark:border-slate-800/80 shadow-sm space-y-3">
              <h3 className="text-[15px] font-bold text-slate-900 dark:text-white uppercase tracking-wider border-b border-slate-100 dark:border-slate-800 pb-2">
              ...
              <div className="bg-blue-50/50 dark:bg-blue-950/10 p-3 rounded-[10px] border border-blue-100/50 dark:border-blue-900/20 text-[13px] font-medium text-blue-800 dark:text-blue-300 leading-snug">
        ...
        {activeTab === 'Engineering' && (
          <div className="space-y-7 animate-in slide-in-from-bottom-2 duration-300">
            ...
            {/* Section heading: Engineering (Size: 20 px) */}
            <div className="space-y-3">
              <h2 className="text-[20px] font-bold text-slate-900 dark:text-white uppercase tracking-tight flex items-center gap-2">
            ...
            <div className="bg-white dark:bg-slate-950 p-4 rounded-[12px] border border-slate-100 dark:border-slate-800/80 shadow-sm space-y-4">
              <div className="text-center">
                <span className="text-[10px] font-black tracking-widest text-slate-400 dark:text-slate-500 uppercase">Interactive Double-Decker Schematic</span>
                <p className="text-[13px] font-bold text-slate-500 dark:text-slate-400 mt-0.5">Click a hotspot badge to view specs</p>
              </div>

              {/* Dynamic Map Canvas / Schematic */}
              <div className="h-64 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800/50 rounded-[10px] relative overflow-hidden flex flex-col justify-between p-4 shadow-inner">
                ...
                <div className="absolute bottom-0 left-0 right-0 h-[28%] bg-gradient-to-t from-sky-300/40 to-sky-100/10 dark:from-sky-950/50 dark:to-slate-900/10 border-t border-sky-200/50 dark:border-sky-900/30 flex items-end justify-center p-1.5 z-0">
                  <span className="text-[12px] font-bold text-sky-700/60 dark:text-sky-400/40 tracking-widest uppercase">Padma River Flow</span>
                </div>

                {/* Drawn Bridge structure using beautiful absolute positioning */}
                <div className="absolute top-[28%] bottom-[25%] left-[12%] right-[12%] flex flex-col justify-center relative">
                  ...
                  <div className="h-2 bg-slate-700 dark:bg-slate-800 rounded-full w-full relative flex items-center justify-between z-10">
                    <span className="absolute -top-4 left-1/2 -translate-x-1/2 text-[12px] font-medium text-slate-400 bg-white dark:bg-slate-950 px-1 rounded border border-slate-100 dark:border-slate-900 scale-90">Upper Roadway</span>
                  </div>
                  ...
                  <div className="h-10 w-full flex items-center justify-between relative px-2 my-1 border-y border-dashed border-slate-300 dark:border-slate-800">
                    ...
                    <div className="absolute inset-0 flex justify-around items-center opacity-30 select-none pointer-events-none">
                      {[...Array(6)].map((_, i) => (
                        <span key={i} className="text-slate-400 font-mono text-[22px]">/\</span>
                      ))}
                    </div>
                    <span className="text-[13px] font-bold text-slate-400 dark:text-slate-500 w-full text-center tracking-widest">WARREN STEEL TRUSS</span>
                  </div>
                  ...
                  <div className="h-2.5 bg-yellow-600/30 border-y-2 border-yellow-700/40 w-full relative flex items-center justify-center z-10">
                    <span className="text-[12px] font-medium text-yellow-800 dark:text-yellow-500 scale-90">Lower Railway</span>
                  </div>

                  {/* Steel Piers/Piles descending into riverbed */}
                  <div className="absolute top-full left-[25%] -translate-x-1/2 h-16 w-3 bg-slate-400 dark:bg-slate-700 shadow-inner rounded-b flex flex-col justify-end items-center">
                    <div className="h-10 w-1 bg-red-500/50 absolute top-full"></div>
                  </div>
                  <div className="absolute top-full left-[75%] -translate-x-1/2 h-16 w-3 bg-slate-400 dark:bg-slate-700 shadow-inner rounded-b flex flex-col justify-end items-center">
                    <div className="h-10 w-1 bg-red-500/50 absolute top-full"></div>
                  </div>
                </div>
                ...
                {ENGINEERING_HOTSPOTS.map((hot) => {
                  ...
                  return (
                    ...
                    >
                      <span className="text-[13px] px-1">{hot.icon}</span>
                      <span className="text-[12px] font-black pr-1 hidden xs:inline">{hot.name.split(' ')[0]}</span>
                    </button>
                  );
                })}
              </div>
              ...
              <div className="bg-slate-50 dark:bg-slate-900 p-4 rounded-[10px] border border-slate-100 dark:border-slate-800/80 space-y-2 transition-colors duration-300">
                <div className="flex justify-between items-center border-b border-slate-200/50 dark:border-slate-800 pb-2">
                  <span className="text-[14px] font-bold text-blue-700 dark:text-blue-400 uppercase tracking-wider flex items-center gap-1.5">
                    <span>{currentHotspot.icon}</span> {currentHotspot.name}
                  </span>
                  <span className="text-[12px] font-normal text-slate-400 dark:text-slate-500 uppercase">Hotspot Specification</span>
                </div>
                <p className="text-[14px] text-slate-700 dark:text-slate-300 font-normal leading-relaxed">{currentHotspot.details}</p>
                <p className="text-[13px] text-blue-700 dark:text-blue-400 font-bold leading-relaxed">{currentHotspot.bnDetails}</p>
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'Timeline' && (
          <div className="space-y-7 animate-in slide-in-from-bottom-2 duration-300">
            ...
            {/* Section heading: Timeline (Size: 20 px) */}
            <div className="space-y-3">
              <h2 className="text-[20px] font-bold text-slate-900 dark:text-white uppercase tracking-tight flex items-center gap-2">
                <span>📅</span> TIMELINE
              </h2>
            ...
            <div className="relative pl-6 border-l-2 border-blue-500/30 dark:border-blue-900/30 space-y-8 py-2">
              {TIMELINE.map((evt, idx) => (
                <div key={idx} className="relative space-y-1.5">
                  {/* Timeline bullet */}
                  <span className="absolute -left-[31px] top-1 w-4.5 h-4.5 rounded-full bg-blue-600 border-4 border-white dark:border-slate-900 shadow-sm flex items-center justify-center"></span>
                  
                  <div className="flex justify-between items-baseline">
                    {/* Year: 17 px bold */}
                    <span className="text-[17px] font-bold text-blue-600 dark:text-blue-400">
                      {evt.year}
                    </span>
                    <span className="text-[12px] font-normal text-slate-400 dark:text-slate-500">Stage {idx + 1}</span>
                  </div>
                  ...
                  <p className="text-[14px] font-normal text-slate-600 dark:text-slate-300 leading-relaxed">
                    {evt.desc}
                  </p>
        ...
        {/* TAB 4: GK QUIZ */}
        {activeTab === 'Quiz' && (
          <div className="bg-white dark:bg-slate-950 p-5 rounded-[12px] border border-slate-100 dark:border-slate-800/80 shadow-sm animate-in slide-in-from-bottom-2 duration-300 transition-colors duration-300">
            
            {!quizFinished ? (
              <div className="space-y-4">
                <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-2">
                  <div>
                    <h3 className="text-[15px] font-bold text-slate-900 dark:text-white">Topic Cadet Quiz</h3>
                    <p className="text-[12px] font-normal text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-0.5">Test your Padma Bridge knowledge</p>
                  </div>
                  <span className="text-[12px] font-bold text-blue-700 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/40 border border-blue-100 dark:border-blue-900/30 px-2.5 py-0.5 rounded-full">
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-[13px] text-blue-700 dark:text-blue-400 font-bold leading-relaxed">
                ...
                <div className="flex flex-col gap-2 pt-2">
                  {QUIZ[quizIdx].options.map((opt, i) => {
                    ...
                    let btnStyle = 'bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-850 text-slate-700 dark:text-slate-300 hover:border-slate-300 dark:hover:text-slate-700';
                    if (isSelected) {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full p-3 rounded-[10px] border text-[13px] font-bold text-left transition flex justify-between items-center cursor-pointer ${btnStyle}`}
                      >
                        <span>{opt}</span>
                ...
                {!quizSubmitted ? (
                  <button
                    ...
                    className={`w-full py-2.5 rounded-[10px] text-[13px] font-bold transition ${
                      selectedOpt !== null 
                  ...
                  <div className="space-y-3">
                    <div className="bg-slate-50 dark:bg-slate-900 p-3 rounded-[10px] border border-slate-100 dark:border-slate-800 text-[13px] text-slate-650 dark:text-slate-400 font-semibold leading-relaxed">
                    ...
                    <button
                      ...
                      className="w-full py-2.5 rounded-[10px] bg-blue-600 hover:bg-blue-700 text-white text-[13px] font-bold transition flex items-center justify-center gap-1 cursor-pointer"
                    >
              ...
              <div className="flex flex-col items-center text-center space-y-4 py-4 animate-in zoom-in duration-300">
                ...
                <div>
                  <h3 className="text-[15px] font-bold text-slate-900 dark:text-white">Bridge GK Mastered!</h3>
                  <p className="text-[13px] text-slate-500 dark:text-slate-400 font-bold mt-0.5">You scored {quizScore} out of {QUIZ.length}</p>
                </div>
                
                <div className="bg-blue-50 dark:bg-blue-950/20 p-3 rounded-[10px] border border-blue-100 dark:border-blue-900/30 w-full text-[13px] font-bold text-blue-800 dark:text-blue-400 flex items-center justify-center gap-2">
                ...
                <div className="flex gap-2.5 w-full">
                  <button 
                    id="retry-quiz-btn"
                    onClick={resetQuiz}
                    className="flex-1 py-2 rounded-[10px] border border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 text-[13px] font-bold text-slate-700 dark:text-slate-300 transition cursor-pointer"
                  >
                  ...
                  <button 
                    ...
                    className="flex-1 py-2 rounded-[10px] bg-blue-600 hover:bg-blue-700 text-white text-[13px] font-bold transition cursor-pointer"
                  >
                    Back to Facts
                  </button>
                </div>
        ...
        <footer className="text-center text-[12px] text-slate-400 dark:text-slate-500 border-t border-slate-100 dark:border-slate-800 pt-4 pb-2">
