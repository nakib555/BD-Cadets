...
export default function Study() {
  ...
  const notes = [
    { title: 'Algebraic Expressions - Key Formulas', subject: 'Mathematics', type: 'PDF Note', icon: 'fa-regular fa-file-pdf', color: 'text-red-500 dark:text-red-400', bg: 'bg-red-50 dark:bg-red-950/30', cardBorder: 'border-[#e3c7c7]', iconBorder: 'border-[#dcadae]' },
    { title: 'Vocabulary Builder Part 1', subject: 'English', type: 'Flashcards', icon: 'fa-regular fa-copy', color: 'text-blue-500 dark:text-blue-400', bg: 'bg-blue-50 dark:bg-blue-950/30', cardBorder: 'border-[#cdd8e4]', iconBorder: 'border-[#c7d0dd]' },
    { title: 'History of Bangladesh - Timeline', subject: 'General Knowledge', type: 'Summary', icon: 'fa-solid fa-list', color: 'text-emerald-500 dark:text-emerald-400', bg: 'bg-emerald-50 dark:bg-emerald-950/30', cardBorder: 'border-[#d8dfe7]', iconBorder: 'border-[#d1e6da]' },
  ];
  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-900 min-h-full pb-6 transition-colors duration-300 animate-in fade-in duration-300 flex flex-col">
      ...
      {/* Tabs */}
      <div className="bg-white dark:bg-slate-950 flex px-4 pt-3 pb-3 gap-2 border-b border-[#d8dfe7] dark:border-slate-800/60 sticky top-[60px] z-10 transition-colors duration-300">
          {tabs.map(tab => (
              <button 
                  key={tab}
      ...
      <div className="p-4 space-y-3 flex-1">
          {activeTab === 'Notes' && (
              <div className="pb-3 space-y-2.5">
                  <div className="flex justify-between items-center">
                      ...
                      <span className="text-[10px] bg-emerald-50 text-emerald-600 dark:bg-emerald-950/40 dark:text-emerald-400 px-2 py-0.5 rounded-full font-bold">New Features</span>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                      ...
                      >
                          <div className="flex justify-between items-start">
                              ...
                              <span className="text-[9px] font-black uppercase text-emerald-600 dark:text-emerald-400 bg-white dark:bg-slate-950 border border-emerald-100 dark:border-emerald-900/30 px-1.5 py-0.5 rounded-[10px]">Biology</span>
                          </div>
                          <div>
                              <h4 className="text-[13px] font-[Georgia] font-black text-slate-900 dark:text-white leading-tight">Photosynthesis</h4>
                              <p className="text-[10px] text-slate-500 dark:text-slate-400 font-bold mt-0.5">Interactive Infographic</p>
                          </div>
                      </div>
                      ...
                      >
                          <div className="flex justify-between items-start">
                              ...
                              <span className="text-[9px] font-black uppercase text-blue-600 dark:text-blue-400 bg-white dark:bg-slate-950 border border-blue-100 dark:border-blue-900/30 px-1.5 py-0.5 rounded-[10px]">Geography</span>
                          </div>
                          <div>
                              <h4 className="text-[13px] font-[Georgia] font-black text-slate-900 dark:text-white leading-tight">Bangladesh Map</h4>
                              <p className="text-[10px] text-slate-500 dark:text-slate-400 font-bold mt-0.5">Divisions & Landmarks</p>
                          </div>
                      </div>
                      ...
                      >
                          <div className="flex justify-between items-start">
                              ...
                              <span className="text-[9px] font-black uppercase text-blue-600 dark:text-blue-400 bg-white dark:bg-slate-950 border border-blue-100 dark:border-blue-900/30 px-1.5 py-0.5 rounded-[10px]">General GK</span>
                          </div>
                          <div>
                              <h4 className="text-[13px] font-[Georgia] font-black text-slate-900 dark:text-white leading-tight">Padma Bridge Infographic</h4>
                              <p className="text-[10px] text-slate-500 dark:text-slate-400 font-bold mt-0.5">Interactive Multi-Decker Layout & Quiz</p>
                          </div>
                      </div>
                  </div>
              </div>
          ...
          {notes.map((note, i) => (
              <div key={i} className={`bg-white dark:bg-slate-950 p-3.5 rounded-[10px] border ${note.cardBorder} dark:border-slate-800/80 shadow-sm flex items-center justify-between group cursor-pointer hover:border-blue-200 dark:hover:border-blue-500/50 transition duration-300`}>
                  <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 ${note.bg} border ${note.iconBorder} rounded-[10px] flex items-center justify-center`}>
                          <i className={`${note.icon} ${note.color} text-lg`}></i>
                      </div>
                      <div>
                          <h3 className="text-[13px] font-[Georgia] font-black text-slate-900 dark:text-white leading-tight mb-0.5">{note.title}</h3>
                  ...
                  <button className="w-[38px] h-[38px] rounded-full bg-slate-50 dark:bg-slate-900 text-slate-400 dark:text-slate-500 flex items-center justify-center group-hover:bg-blue-50 dark:group-hover:bg-blue-950/40 group-hover:text-blue-500 dark:group-hover:text-blue-400 transition">
